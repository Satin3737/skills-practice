...
const planets: FastifyPluginAsyncZod = async (fastify): Promise<void> => {
    ...
    fastify.get('/', {schema: getPlanetsSchema}, async (req, res): Promise<void> => {
        void res.send(await planetsService.getPlanets(req.query));
    });
    ...
    fastify.post(
        ...
        async (req, res): Promise<void> => {
            void res.code(201).send({planet: await planetsService.createPlanet(req.body)});
        }
    );

    fastify.get('/:id', {schema: getPlanetSchema}, async (req, res): Promise<void> => {
        void res.send({planet: await planetsService.getPlanetById(req.params.id)});
    });
    ...
    fastify.patch(
        ...
        async (req, res): Promise<void> => {
            void res.send({planet: await planetsService.updatePlanet(req.params.id, req.body)});
        }
    );
    ...
    fastify.delete(
        ...
        async (req, res): Promise<void> => {
            void res.send({planet: await planetsService.deletePlanet(req.params.id)});
        }
    );
    ...
    fastify.get(
        ...
        async (req, res): Promise<void> => {
            void res.send(await missionsService.getMissionsByPlanet(req.params.id, req.query));
        }
    );
    ...
    fastify.post(
        ...
        async (req, res): Promise<void> => {
            ...
            void res.code(201).send({missions});
        }
    );
};
